...
export function Navbar({ locale }: NavbarProps) {
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-canvas border-b border-hairline">
      <nav className="mx-auto flex h-16 max-w-300 items-center justify-between px-6 lg:px-8">
        ...
        <div className="hidden lg:flex items-center gap-2">
          ...
          <Link
            href={`/${locale}/#contact`}
            className="inline-flex items-center justify-center h-9 px-4 rounded-ds-md bg-ink text-canvas font-sans text-sm font-medium transition-colors hover:bg-surface-light ms-1"
          >
            {t("getStarted")}
          </Link>
        </div>
      ...
      {/* Mobile menu */}
      {open && (
        <div className="lg:hidden bg-surface-card border-t border-hairline px-6 py-6">
